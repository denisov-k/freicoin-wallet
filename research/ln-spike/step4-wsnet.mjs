// step4: то же, что step2, но LN-транспорт — БРАУЗЕРНЫЙ: SocketDescriptor поверх стандартного
// WebSocket (ws-net.mjs) через наш ws↔tcp мост (порт 3070 → alice:9741). Проходит — значит
// сокет-слой кошелька готов, в браузере меняется ровно ничего.
// Запуск: node step4-wsnet.mjs (нужен мост: FW_NODE_PORT=9741 FW_BRIDGE_PORT=3070 bridge.mjs)
