// step6: как step5, но блоки разбирает НАШ btc-block.mjs (браузерный сплиттер сегвита без
// lock_height), а не bitcoin-cli JSON — то, что реально исполнит воркер. bitcoin-cli остаётся
// только источником сырых байтов (в бою — neutrino по P2P).
// (— заголовок step5 ниже —)
...
import { parseBtcBlock } from '../../apps/web/src/services/light/net/btc-block.mjs';
...
const feedTo = async height => {
  while (tipHeight < height) {
    ...
    const rawBlk = Buffer.from(btcli('getblock', tipHash, '0'), 'hex');
    const { txs } = await parseBtcBlock(new Uint8Array(rawBlk));   // ← НАШ сплиттер
    const relevant = [];
    for (const t of txs) if (chain.isRelevant({ txid: t.txid, outs: t.outs })) relevant.push({ index: t.index, raw: Buffer.from(t.raw) });
...
const mine = async n => { btcli('generatetoaddress', String(n), (await aliceA('newaddress', 'p2wkh')).address); await feedTo(+btcli('getblockcount')); };
...
console.log(claimed && pay.payment_preimage === R.toString('hex') ? 'STEP6 OK ✅' : 'STEP6 FAILED');
