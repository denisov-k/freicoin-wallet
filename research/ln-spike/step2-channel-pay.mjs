// step2: входящий канал от LSP (LND-Алиса) на LDK-узел, фид цепи через Listen, инвойс под
// ВНЕШНИЙ хеш H, платёж Алисы, УДЕРЖАНИЕ (PaymentClaimable без клейма) и claim_funds(R) —
// hold-семантика, на которой стоит атомарный реверс свопа.
// Запуск: node step2-channel-pay.mjs
...
const ensurePeer = async () => {   // LND роняет молчаливого пира — переподключаемся при нужде
  for (let i = 0; i < 50; i++) {
    ...
    if (i % 10 === 5) { try { await net.connect_peer('127.0.0.1', 9741, Buffer.from(ALICE_PK, 'hex')); } catch {} }
...
const feedTo = height => {
  while (tipHeight < height) {
    ...
    const raw = Buffer.from(btcli('getblock', tipHash, '0'), 'hex');
...
console.log(claimed && pay.payment_preimage === R.toString('hex') ? 'STEP2 OK ✅' : 'STEP2 FAILED');
