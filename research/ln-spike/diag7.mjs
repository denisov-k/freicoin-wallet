...
{ // ДИАГ: обёртки над Filter-регистрациями
  const f = chain.filter;
  const origRegTx = f.register_tx?.bind(f);
}
const _watch0 = chain._watch.bind(chain);
chain._watch = (spk) => { console.log('  [REG] _watch spk', (spk||'').slice(0,24)+'…', 'size->', chain.watchedSpks.size+1); return _watch0(spk); };
...
const _tick0 = btc.tick.bind(btc);
let tickN = 0;
btc.tick = async () => { if (tickN++ % 10 === 0) console.log('  [TICK] watchedSpks', chain.watchedSpks.size, 'txids', chain.watchedTxids.size, 'scanned', btc.scanned, 'headers', btc.headers.length, 'rescanFrom', btc._rescanFrom); return _tick0(); };
...
console.log('-- alice opens a 300k channel…'); const _diagStart = Date.now();
