...
export function paintActivity(txs, final = true) {
  ...
  const rowHtml = i =>
    ...
    : `<div class="act-a ${(+i.amount) < 0 ? 'neg' : 'pos'}"><span>${amtStr(i)}</span>${isImmatureGen(i) ? `<span class="sub immature">${tr('immature')} ${i.confirmations}/${COINBASE_MATURITY}</span>` : i.confirmations === 0 ? `<span class="sub immature">${tr('confirming…')}</span>` : i.note ? `<span class="sub">${tr(i.note)}</span>` : ''}</div>`}`;
