...
import { holdingLabel, isTickerId, isPlotId, tickerId, validTicker } from '@core/freiland.mjs';
...
export function renderExchange(el) {
  ...
  el.innerHTML = `
    ...
    ${nv3 ? `<div id="mktHold"${mktClass === 'hold' ? '' : ' hidden'}>
      ...
      <div class="sub" id="covKindHint" style="font-size:12px;margin:2px 0">${tr('Type a name to find it: the registry stores only hashes, so there is nothing to browse. A name found is a name you can buy at the price its holder set.')}</div>
  ...
  if (nv3) {
    ...
    if (cov) {
      ...
      const paintCovKind = () => {
        ...
        if (hint) hint.textContent = tr(plot
