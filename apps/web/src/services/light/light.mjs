  ...
  const toCache = (r, stale = false) => {
    ...
    return {
      ...
      utxos: r.utxos.filter(u => !u.assetTag || u.assetTag === '0'.repeat(40)).map(u => ({
        ...
        coinbase: u.coinbase, scriptPubKey: u.script, unconfirmed: !!u.unconfirmed,   // pending-change coin: spendable (own chained tx)
      })),
      ...
      // assetUtxos backs coin selection for Issue/Exchange (offers, HTLCs) — those must settle on
      // CONFIRMED coins, so the unconfirmed pending-change outputs are excluded here (they still ride
      // the plain-FRC `utxos` above, so the FRC Send flow can chain onto them).
      assetUtxos: r.utxos.filter(u => !u.unconfirmed).map(u => ({ outpoint: `${u.txid}:${u.vout}`, spk: u.script, assetTag: (!u.assetTag || u.assetTag === '0'.repeat(40)) ? null : u.assetTag, value: String(u.value), refheight: u.refheight, coinbase: !!u.coinbase, ...(u.tokenHash ? { tokens: u.tokens ?? [], tokenHash: u.tokenHash } : {}) })),
